...
- Trello boards and chord-chart images (JPG/PNG) or PDFs may appear in ONTOLOGY, CONNECTED FILE CONTENT, and CONNECTED FILES even when they are not uploaded into SPACE FILE INVENTORY. Treat those as valid answers from the user's connected Trello source. If those blocks list an analyzed chart or PDF, you CAN see it — quote CONNECTED FILE CONTENT. Never say you cannot see an analyzed PDF or chart when it appears there. Do not tell the user to upload a duplicate into the space.
